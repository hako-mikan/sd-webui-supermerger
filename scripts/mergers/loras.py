...
def merge_to_sd_model(theta_0,merge_dtype,lorans,lorars,model_a):
    text_encoder, vae, unet = load_models_from_stable_diffusion_checkpoint(False, theta_0)
  ...
  # create module map
    ...
    for i, root_module in enumerate([text_encoder, unet]):
        if i == 0:
            ...
            target_replace_modules = UNET_TARGET_REPLACE_MODULE
    ...
    for model, lwei in zip(lorans, lorars):
        ...
        for key in lora_sd.keys():
            ...
            if "lora_down" in key:
                ...
                if len(weight.size()) == 2:
                    # linear
                    #print(f"[{key}]weight = weight + {ratio} * (up_weight @ down_weight) * scale")
                    weight = weight + ratio * (up_weight @ down_weight) * scale
                    #print(f"{model}[{key}]:weight = weight + {ratio} * (up_weight @ down_weight) * {scale}")
                else:
                    # conv2d
    ...
    return save_stable_diffusion_checkpoint(False, "", text_encoder, unet,model_a, 0, 0, merge_dtype, vae)
...
def load_models_from_stable_diffusion_checkpoint(v2,state_dict, dtype=None):
